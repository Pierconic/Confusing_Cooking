...
public class PlaceCheeseBucketProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack) {
		...
		double raytrace_x = 0;
		double raytrace_z = 0;
		if ((entity instanceof LivingEntity _livEnt ? _livEnt.getOffhandItem() : ItemStack.EMPTY).getItem() == CookingWithMindthemoodsModItems.CHEESE_BUCKET.get()) {
			if (entity.level().clip(new ClipContext(entity.getEyePosition(1f), entity.getEyePosition(1f).add(entity.getViewVector(1f).scale(5)), ClipContext.Block.OUTLINE, ClipContext.Fluid.ANY, entity)).getType() == HitResult.Type.BLOCK
				...
				if (world.isEmptyBlock(BlockPos.containing(raytrace_x, raytrace_y + 1, raytrace_z))) {
					...
					world.setBlock(BlockPos.containing(raytrace_x, raytrace_y + 1, raytrace_z), CookingWithMindthemoodsModBlocks.CHEESE_WHEEL.get().defaultBlockState(), 3);
		...
		if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == CookingWithMindthemoodsModItems.CHEESE_BUCKET.get()) {
			if (entity.level().clip(new ClipContext(entity.getEyePosition(1f), entity.getEyePosition(1f).add(entity.getViewVector(1f).scale(5)), ClipContext.Block.OUTLINE, ClipContext.Fluid.ANY, entity)).getType() == HitResult.Type.BLOCK
					&& !(new Object() {
						public boolean checkGamemode(Entity _ent) {
							if (_ent instanceof ServerPlayer _serverPlayer) {
