...
public class RiceStage1GrowthProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		if (Math.random() < 0.1 && (world.getBlockState(BlockPos.containing(x, y, z))).getFluidState().isSource()) {
			if ((world.getBlockState(BlockPos.containing(x, y, z))).getFluidState().isSource() && (world.getBlockState(BlockPos.containing(x, y, z))).getBlock() == CookingWithMindthemoodsModBlocks.RICE_BOTTOM_STAGE_1.get()) {
				world.setBlock(BlockPos.containing(x, y, z), Blocks.AIR.defaultBlockState(), 3);
				if (world instanceof ServerLevel _serverworld) {
					StructureTemplate template = _serverworld.getStructureManager().getOrCreate(ResourceLocation.fromNamespaceAndPath("cooking_with_mindthemoods", "medium_rice"));
					if (template != null) {
